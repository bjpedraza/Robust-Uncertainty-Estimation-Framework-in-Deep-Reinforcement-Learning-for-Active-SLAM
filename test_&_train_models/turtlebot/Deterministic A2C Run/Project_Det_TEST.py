...
import Network_Model_Det_TEST as net
...
if __name__ == '__main__':
    ...
    model_A2C.load_weights('/home/bryan/Documents/Trad A2C GOOD/turtle_c2c_Actor_ep300_train')
    ...
    wandb.init(entity = "bpedraz4", project="TEST_Tradional_A2C_{}_lr_{}_kfold".format(total_episodes, learningRate))
    ...
    for x in range(1, total_episodes, 1):
        ...
        for i in range(steps): # <----- steps
            ...
            actions[batch_step], values[batch_step], snr_signal = agent.action_value(observation)
            ...
            if (batch_step == 15): #reset parameters
                batch_step = -1
                #mem_observation = np.zeros(( batch_sz, 32,32,1))
                #actions = np.zeros(batch_sz)
                #values = np.zeros(batch_sz)
