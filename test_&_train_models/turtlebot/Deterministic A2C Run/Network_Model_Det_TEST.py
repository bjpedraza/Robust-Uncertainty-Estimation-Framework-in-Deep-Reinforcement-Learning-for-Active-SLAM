...
class A2CAgent:
    ...
    @tf.function    
    def create_adversarial_pattern(self, input_image): # reward, done, value, next_value):
          with tf.GradientTape() as tape:
            tape.watch(input_image)
            self.model.trainable = False

            logits, value = self.model(input_image)
            action = self.model.dist(logits)

            #action = tf.cast(action, tf.float32)
            policy_loss = self.weighted_sparse_ce(action, logits) #, sample_weight=advantages)
            probs = tf.nn.softmax(logits)
            entropy_loss = kls.categorical_crossentropy(probs, probs)

            loss =  (policy_loss - self.entropy_c * entropy_loss) #+ (self.value_c * kls.mean_squared_error(returns, value))
    ...
    def test_on_batch_dist(self,x):  
        self.model.dist.trainable = False                    
        action = self.model.dist(x, training=False)            
        return action
    ...
    def action_value(self, obs):
        ...
        #value = self.dist2.predict_on_batch(obs)
        ...
        #Gaussian:
        gauss = np.random.normal(0, 0.0001, obs.shape)
        gauss = gauss.reshape(obs.shape)
        gauss_obs = obs + gauss


        logits, value = self.test_on_batch(gauss_obs)
        action = self.test_on_batch_dist(logits)
        #action = self.model.dist.predict_on_batch(logits)
        ...
        self.mu_out_[self.test_no_steps, :, :] = logits
        ...
        return action, value[0], self.snr_signal
