...
class GazeboCircuit2TurtlebotLidarEnv(gazebo_env.GazeboEnv):
    ...
    def __init__(self):
        ...
        #self.std_noise = 0.001
        self.std_noise = 0.5


    '''
    @tf.function    
    def create_adversarial_pattern(input_image, encodeAction):
          with tf.GradientTape() as tape:
            tape.watch(input_image)
            cnn_model.trainable = False 
            prediction, sigma = cnn_model(input_image) 
            loss_final = nll_gaussian(encodeAction, prediction,  tf.clip_by_value(t=sigma, clip_value_min=tf.constant(-1e+3),
    ...
    def step(self, action):
        ...
        if not done:
            if action == 0: #go forward
                reward = 1.2 + math.tanh(0.01 / entropyVal.data)
            else: # turn
                reward = -0.10 + math.tanh(0.01 / entropyVal.data)
        else:
            reward = -100
        ...
        '''
        #EDIT Each noise will be seperate run
	if epoch < 25:
        elif epoch < 50:
            #ADD Noise to Image: 
            gauss = np.random.normal(0, self.std_noise , cv_image.shape)
            gauss = gauss.reshape(cv_image.shape)
            cv_image = cv_image + gauss
        elif epoch < 100:
            #ADD Noise to Image:
            gauss = np.random.normal(0, self.std_noise*100 , cv_image.shape)
            gauss = gauss.reshape(cv_image.shape)
            cv_image = cv_image + gauss
        elif epoch < 150:
            #ADD Noise to Image:
            gauss = np.random.normal(0, self.std_noise*200 , cv_image.shape)
            gauss = gauss.reshape(cv_image.shape)
            cv_image = cv_image + gauss
        else:
            #ADD Noise to Image:
            gauss = np.random.normal(0, self.std_noise*300 , cv_image.shape)
            gauss = gauss.reshape(cv_image.shape)
            cv_image = cv_image + gauss
        '''
        gauss = np.random.normal(0, self.std_noise , cv_image.shape)
